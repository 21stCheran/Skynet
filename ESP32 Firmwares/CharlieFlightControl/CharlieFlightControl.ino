...
const unsigned int RC_HEARTBEAT_INTERVAL = 10; // 10ms -> 100Hz (much faster response)
...
void executeSafeDisarm() {
    Serial.println("SAFE DISARM: Initiating safe disarm sequence");
    ...
    Serial.printf("SAFE DISARM: Started with throttle %d, will disarm in %lu ms\n", 
                 SAFE_THROTTLE_MIN, SAFE_DISARM_DELAY);
}
...
void handleSafeDisarm() {
    ...
    if (elapsed >= SAFE_DISARM_DELAY) {
        // Complete the disarm
        Serial.println("SAFE DISARM: Completing disarm sequence");
        setRCChannel(2, RC_MIN);    // Throttle minimum
        setRCChannel(4, RC_MIN);    // Disarm
        safeDisarmState.inProgress = false;
        currentCommand = {"", 0, 0, false, true}; // Clear current command
        ...
        // Optional: Gradually reduce throttle over time
        // uint16_t targetThrottle = SAFE_THROTTLE_MIN - ((elapsed * (SAFE_THROTTLE_MIN - RC_MIN)) / SAFE_DISARM_DELAY);
        // setRCChannel(2, max(targetThrottle, RC_MIN));
    }
}
...
void executeSafeHover(int throttleValue, bool safeMode) {
    ...
    if (safeMode) {
        ...
        Serial.printf("SAFE HOVER: Input=%d, Safe=%d (enforced limits %d-%d)\n", 
                     throttleValue, safeThrottle, SAFE_THROTTLE_MIN, SAFE_THROTTLE_MAX);
    } else {
        // Still enforce absolute minimum when armed
        safeThrottle = max(throttleValue, (int)(RC_MIN + 50)); // At least 50 above minimum
        Serial.printf("UNSAFE HOVER: Input=%d, Limited=%d (minimum motor speed enforced)\n", 
                     throttleValue, safeThrottle);
    }
...
void executeSafeMovement(String direction, int intensity, bool safeMode) {
    ...
    if (safeMode) {
        ...
        Serial.printf("SAFE MOVEMENT: %s intensity %d->%d (safe limit %d)\n", 
                     direction.c_str(), intensity, safeIntensity, SAFE_MOVEMENT_MAX);
    } else {
        Serial.printf("UNSAFE MOVEMENT: %s intensity %d (no limits)\n", 
                     direction.c_str(), safeIntensity);
    }
...
void parseAndExecuteCommand(String jsonCommand) {
    ...
    if (command == "hover") {
        ...
        Serial.printf("ARMING: Drone armed (Safe Mode: %s)\n", safeMode ? "ON" : "OFF");
    } else if (command == "disarm") {
        executeEmergencyStop(); // Standard immediate disarm
        Serial.println("DISARMING: Drone disarmed (immediate)");
    } else if (command == "safe_disarm") {
        executeSafeDisarm(); // NEW: Safe disarm with motor speed maintenance
        Serial.println("SAFE DISARMING: Drone safe disarm initiated");
    } else {
...
void loop() {
    ...
    handleSafeDisarm(); // NEW: Handle safe disarm state machine
    ...
    // === Part 7: Request Telemetry Periodically ===
    if (clientConnected) {
        requestTelemetry();
    }
}